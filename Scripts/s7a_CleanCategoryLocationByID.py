...
df = pd.read_csv("/data/processed/final_cleaned_with_category_location.csv")

# Keep only rows with a non-null Listing ID
df_cleaned = df[df["Listing ID"].notna()]

# Save result
df_cleaned.to_csv("/data/processed/final_cleaned_with_category_location_cleaned.csv", index=False)
