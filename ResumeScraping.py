...
with open("active_links_only.txt", "r", encoding="utf-8") as f:
    all_urls = [line.strip() for line in f.readlines()]

# Step 2: Load already scraped URLs from existing CSV
try:
    scraped_df = pd.read_csv("active_listings_with_prices_and_features.csv")
...
from ScrapingAllDataFromLinksBeta import scrape_and_append_listing
